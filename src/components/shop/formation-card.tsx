...
export function FormationCard({ formation, dict, lang }: { formation: Formation, dict: Dictionary['shop'], lang: Locale }) {
    ...
    const localizedName = formation.name?.[lang] || formation.name?.en || 'Formation sans nom';
    ...
    return (
        <Link href={`/${lang}/formations/${formation.id}`} className="flex group">
            <Card className="flex flex-col w-full overflow-hidden transition-shadow duration-300 group-hover:shadow-xl">
