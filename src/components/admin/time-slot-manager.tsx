...
type TimeSlot = {
    ...
    maxParticipants: number; // <--- AJOUTÉ ICI
};
...
export default function TimeSlotManager({ sessionTypeId, dictionary, lang }: TimeSlotManagerProps) {
    ...
    const handleAddSlot = async (data: TimeSlotFormData) => {
        if (!firestore || !sessionType) return; // On vérifie que le sessionType est bien chargé
        ...
        // Récupération de la capacité MAX du modèle
        ...
        for (let i = 0; i < effectiveRepeatCount; i++) {
            ...
            slotsToAdd.push({
                ...
                maxParticipants: maxParticipants, // <--- ON COPIE LA VALEUR
            });
        }
                                    ...
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            ...
                                            <div className="flex gap-2 text-sm text-muted-foreground">
                                                <span>{`→ ${format(slot.endTime.toDate(), 'p', { locale })}`}</span>
                                                <span>•</span>
                                                {/* Affichage Admin pour vérifier */}
                                                <span>{slot.bookedParticipantsCount} / {slot.maxParticipants || sessionType?.maxParticipants || 1}</span>
                                            </div>
                                        </div>
                                    </div>
                                    <AlertDialog>
