...
import { Loader2, Video, Calendar, Clock, Users, Link as LinkIcon, BookHeart, Download, FileText } from 'lucide-react'; // Ajout FileText
...
import TranscriptModal from '@/components/admin/TranscriptModal'; // Ajout de l'import
...
export default function AdminSessionManager({ lang, dictionary }: { lang: Locale, dictionary: any }) {
  ...
  // Ajout de l'état pour gérer l'ouverture de la modale
  const [selectedSessionIdForTranscript, setSelectedSessionIdForTranscript] = useState<string | null>(null);
        ...
        <Card key={session.id} className="flex flex-col">
            ...
            <CardFooter className="flex flex-col items-stretch gap-2">
                ...
                <Button 
                    ...
                    size="sm" 
                    className="w-full"
                    onClick={() => setSelectedSessionIdForTranscript(session.id)}
                >
        ...
        <div>
            ...
            {pastSessions.length > 0 ? (
                ...
                <div className="text-center py-10 border-2 border-dashed rounded-lg">
                    <p className="text-muted-foreground">Aucune séance passée trouvée.</p>
                </div>
            )}
        </div>

        {/* --- MODALE AJOUTÉE ICI --- */}
        {selectedSessionIdForTranscript && (
            <TranscriptModal 
                isOpen={!!selectedSessionIdForTranscript} 
                onClose={() => setSelectedSessionIdForTranscript(null)} 
                sessionId={selectedSessionIdForTranscript} 
            />
        )}
    </section>
  );
