...
// Import de la fonction utilitaire pour générer le code de l'utilisateur
...
const formSchema = z.object({
  ...
  referredByCode: z.string().optional(), // On accepte un code optionnel
});
        ...
        if (mode === 'signup') {
            ...
            if (signInMethods.length > 0) {
                ...
                toast({
                    ...
                    description: `Email used by ${providerName}.`,
                });
                return;
            }
            ...
            // Enregistrement des données utilisateur avec le code de parrainage saisi
            await setDoc(doc(firestore, "users", user.uid), {
                ...
                referredByCode: data.referredByCode || null, // On stocke le code saisi s'il existe
                hasFirstPurchase: false, // Marqueur pour valider le parrainage plus tard
                gamification: {
                    seeds: 0,
                    totalAccumulated: 0,
                    rank: "Racines",
    ...
    try {
        ...
        if (additionalInfo?.isNewUser) {
            ...
            await setDoc(doc(firestore, "users", user.uid), {
                ...
                tree_score: 0,
                referralCode: generateReferralCode(user.uid),
                referredByCode: null, // Note: Google SignIn ne permet pas de saisir un code au premier clic
    ...
    <Card className="w-full max-w-sm">
      ...
      <CardContent className="grid gap-4">
        <form onSubmit={handleSubmit(onSubmit)} className="grid gap-4">
          ...
          {/* AJOUT DU CHAMP PARRAINAGE DYNAMIQUE */}
          {mode === "signup" && (
            <div className="grid gap-2">
              <Label htmlFor="referredByCode">
                {(dictionary as any).referralLabel || "Code Parrainage"}
              </Label>
              <Input
                id="referredByCode"
                type="text"
                placeholder={(dictionary as any).referralPlaceholder || "SOUL-XXXX"}
