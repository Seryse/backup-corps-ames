...
export default function IrisphereForumPage({ params }: { params: Promise<{ lang: string }> }) {
  ...
  // --- SÉCURITÉ GRIMOIRE ---
  // On construit la requête UNIQUEMENT si le user est là et authentifié
  const threadsQuery = useMemoFirebase(() => {
    if (!firestore || !user) return null;

    return query(
        collection(firestore, 'forum_threads'),
        where('category', '==', 'irisphere-harmonia'),
        orderBy('createdAt', 'desc')
    );
  }, [firestore, user]);

  // Récupération des données (automatique et sécurisée)
    ...
    <div className="bg-[#FDFBF8] min-h-screen">
      <div className="container mx-auto px-4 py-12">
        <header className="text-center mb-12">
          ...
          <h1 className="text-4xl font-light text-gray-800 tracking-wider mt-2">Cercle Irisphère</h1>
          <p className="text-lg text-gray-500 mt-2">Un espace d'échange sacré sur le soin signature Irisphère Harmonia.</p>
        </header>

        <div className="max-w-4xl mx-auto">
            <div className="mb-8 text-right">
                <Button asChild className="bg-amber-600 hover:bg-amber-700 text-white">
                    <Link href={`/${lang}/forum/irisphere-harmonia/nouveau`}>
            ...
            <div className="space-y-4">
                {threads && threads.length > 0 ? (
                    threads.map((thread: any) => {
                         ...
                         return (
                            <Link href={`/${lang}/forum/irisphere-harmonia/${thread.id}`} key={thread.id}>
