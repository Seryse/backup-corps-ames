...
export default function BookingsPage({ params }: { params: Promise<{ lang: Locale }> }) {
  ...
  const { upcomingSessions, pastSessions } = useMemo(() => {
    ...
    const allMerged = sessions
      ...
      // Tri par date de début (décroissant pour le moment)
      .sort((a, b) => b.timeSlot.startTime.toMillis() - a.timeSlot.startTime.toMillis());
    
    // Une séance est "à venir" ou "en cours" tant que la date de FIN n'est pas passée
    ...
    // On inverse 'upcoming' pour avoir la prochaine séance en premier
  ...
  const SessionCard = ({ session }: { session: MergedSession }) => {
      ...
      const endTime = timeSlot.endTime.toDate(); // Récupération de l'heure de fin

      // --- CORRECTION ICI ---
      // On affiche le bouton tant que la séance n'est pas terminée (heure de fin non passée)
      const isJoinable = !isPast(endTime);
            ...
            <CardHeader className="flex-row gap-4 items-start">
                ...
                <div className="flex-grow">
                    ...
                    <CardDescription className="flex items-center gap-2 text-muted-foreground">
                        ...
                        {' - '}
                        {format(endTime, 'p', { locale: dateFnsLocale })}
                    </CardDescription>
                </div>
            </CardHeader>
            {isJoinable && (
