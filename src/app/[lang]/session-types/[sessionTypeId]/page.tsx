...
export default function SessionTypeDetailPage({ params }: { params: Promise<{ lang: Locale, sessionTypeId: string }> }) {
    const { lang, sessionTypeId } = use(params);
    ...
    const sessionTypeRef = useMemoFirebase(() => {
        ...
        return doc(firestore, 'sessionTypes', sessionTypeId);
    }, [firestore, sessionTypeId]);

    // CORRECTION FINALE : Le hook useDoc ne prend pas de deuxième argument. C'était une invention.
    const { data: sessionType, isLoading } = useDoc<SessionType>(sessionTypeRef);
